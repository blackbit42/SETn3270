...
# >>> tn3270.disconnect()
#
# A check function has also been created to check if the server accepts tn3270
# connections.
...
# >>> tn3270.get_binary_file("'ibmuser.asm(compiled)'",
#                            "/home/dade/compiled.asm")
#
#########
#
...
SB = bytes([250])  # Sub-option to follow
WILL = bytes([251])  # Will; request or confirm option begin
WONT = bytes([252])  # Wont; deny option request
DO = bytes([253])  # Do = Request or confirm remote option
DONT = bytes([254])  # Don't = Demand or confirm option halt
IAC = bytes([255])  # Interpret as Command
SEND = bytes([0o01])  # Sub-process negotiation SEND command
IS = bytes([000])  # Sub-process negotiation IS command


# TN3270 Telnet Commands
TN_ASSOCIATE = bytes([0])
TN_CONNECT = bytes([1])
TN_DEVICETYPE = bytes([2])
TN_FUNCTIONS = bytes([3])
TN_IS = bytes([4])
TN_REASON = bytes([5])
TN_REJECT = bytes([6])
TN_REQUEST = bytes([7])
TN_RESPONSES = bytes([2])
TN_SEND = bytes([8])
TN_TN3270 = bytes([40])
TN_EOR = bytes([239])  # End of Record
...
#####
NO = bytes([0x60])  # no aid
QREPLY = bytes([0x61])  # reply
ENTER = bytes([0x7d])  # enter
PF1 = bytes([0xf1])
PF2 = bytes([0xf2])
PF3 = bytes([0xf3])
PF4 = bytes([0xf4])
...
# Structured fields
# From x3270 sf.c
SF_READ_PART = bytes([0x01])  # read partition
SF_RP_QUERY = bytes([0x02])  # query
SF_RP_QLIST = bytes([0x03])  # query list
SF_RPQ_LIST = bytes([0x00])  # QCODE list
SF_RPQ_EQUIV = bytes([0x40])  # equivalent+ QCODE list
SF_RPQ_ALL = bytes([0x80])  # all
SF_ERASE_RESET = bytes([0x03])  # erase/reset
SF_ER_DEFAULT = bytes([0x00])  # default
SF_ER_ALT = bytes([0x80])  # alternate
SF_SET_REPLY_MODE = bytes([0x09])  # set reply mode
SF_SRM_FIELD = bytes([0x00])  # field
SF_SRM_XFIELD = bytes([0x01])  # extended field
SF_SRM_CHAR = bytes([0x02])  # character
SF_CREATE_PART = bytes([0x0c])  # create partition
CPFLAG_PROT = bytes([0x40])  # protected flag
CPFLAG_COPY_PS = bytes([0x20])  # local copy to presentation space
CPFLAG_BASE = bytes([0x07])  # base character set index
SF_OUTBOUND_DS = bytes([0x40])  # outbound 3270 DS
SF_TRANSFER_DATA = bytes([0xd0])  # file transfer open request
...
TN3270E_ASSOCIATE = bytes([0x00])
TN3270E_CONNECT = bytes([0x01])
TN3270E_DEVICE_TYPE = bytes([0x02])
TN3270E_FUNCTIONS = bytes([0x03])
TN3270E_IS = bytes([0x04])
TN3270E_REASON = bytes([0x05])
TN3270E_REJECT = bytes([0x06])
TN3270E_REQUEST = bytes([0x07])
TN3270E_SEND = bytes([0x08])
...
class TN3270:
    ...
    def disable_enhanced(self, disable=True):
        ...
        if disable:
            self.unsupported_opts[bytes([40])] = 'TN3270'
        else:
    ...
    def DECODE_BADDR(self, byte1, byte2):
        """ Decodes Buffer Addresses.
            Buffer addresses can come in 14 or 12 (this terminal doesn't
            support 16 bit) this function takes two bytes (buffer addresses
            are two bytes long) and returns the decoded buffer address."""
        if (byte1 & 0xC0) == 0:
    ...
    def check_tn3270(self, host, port=0, timeout=3):
        ...
        if data == IAC + DO + options['TN3270']:
            ...
            data = sock.recv(256)
            if data != IAC + SB + options['TTYPE'] + \
                    SEND + IAC + SE or data == '':
                return False
            sock.sendall(
                IAC + SB + options['TTYPE'] + IS + DEVICE_TYPE + IAC + SE)
            data = sock.recv(256)
            if data[0:2] == IAC + DO:
                sock.close()
                return True
    ...
    def get_data(self):
        ...
        while not self.first_screen and status:
            try:
                ...
                err = e.args[0]
                # This means the SSL socket timed out, not a regular socket so
                # we catch it here
                if 'timed out' in err:
                    self.msg(1, "recv timed out! We're done here")
                    break
    ...
    def ts_processor(self, data):
        ...
        if self.telnet_state == TNS_DATA:
            ...
            if data in supported_options and not (
                    data in self.unsupported_opts):
                self.msg(1, "<< IAC WILL %s", supported_options[data])
                # if we haven't already replied to this, let's reply
            ...
            if data == SE:
                ...
                if self.state != TN3270E_DATA:
                    ...
                    for i in self.sb_options:
                        if i in telnet_options:
                            phrase += telnet_options[i] + ' '
                        elif i in telnet_commands:
                            phrase += telnet_commands[i] + ' '
                        elif i in supported_options:
                            phrase += supported_options[i] + ' '
                        else:
                            phrase += i + ' '
                ...
                if (self.sb_options[0] == options['TTYPE'] and
                    ...
                    self.send_data(IAC + SB + options['TTYPE'] +
                                   IS + DEVICE_TYPE + IAC + SE)
                elif self.client_options.get(options['TN3270'], False) and \
    ...
    def negotiate_tn3270(self):
        ...
        TN3270_REQUEST = {
            bytes([0]): 'BIND_IMAGE',
            bytes([1]): 'DATA_STREAM_CTL',
            bytes([2]): 'RESPONSES',
            bytes([3]): 'SCS_CTL_CODES',
            bytes([4]): 'SYSREQ'
        }
        ...
        for i in self.sb_options:
            if tn_request and i in TN3270_REQUEST:
                ...
                if i == TN3270E_REQUEST:
                    tn_request = True
            elif i in telnet_options:
                phrase += telnet_options[i] + ' '
            elif i in telnet_commands:
                phrase += telnet_commands[i] + ' '
            elif i in supported_options:
        ...
        if self.sb_options[1] == TN3270E_SEND:
            if self.sb_options[2] == TN3270E_DEVICE_TYPE:
                ...
                if self.connected_lu == '':
                    self.msg(1, ">> IAC SB TN3270 TN3270E_DEVICE_TYPE " +
                                "TN3270E_REQUEST %s IAC SE", DEVICE_TYPE)
                    self.send_data(IAC + SB + options['TN3270'] +
                                   TN3270E_DEVICE_TYPE + TN3270E_REQUEST +
                                   DEVICE_TYPE + IAC + SE)
                else:
                    self.msg(1, ">> IAC SB TN3270 TN3270E_DEVICE_TYPE " +
                                "TN3270E_REQUEST " + DEVICE_TYPE +
                                " CONNECT " + self.connected_lu + " IAC SE")
                    self.send_data(IAC + SB + options['TN3270'] +
                                   TN3270E_DEVICE_TYPE + TN3270E_REQUEST +
                                   DEVICE_TYPE + TN_CONNECT +
                                   self.connected_lu + IAC + SE)
        elif self.sb_options[1] == TN3270E_DEVICE_TYPE:
            if self.sb_options[2] == TN3270E_REJECT:
                self.msg(1, 'Received TN3270E_REJECT after sending LU %s',
                         self.connected_lu)
                return False
            ...
            if CONNECT_option > 1:
                self.connected_lu = self.sb_options[CONNECT_option +
                                                    1:SE_location]
            ...
            self.msg(1, '>> IAC SB TN3270 TN3270E_FUNCTIONS TN3270E_REQUEST '
                        'IAC SE')
            self.send_data(IAC + SB + options['TN3270'] + TN3270E_FUNCTIONS +
                           TN3270E_REQUEST + IAC + SE)
        elif self.sb_options[1] == TN3270E_FUNCTIONS:
            if self.sb_options[1] != TN3270E_IS:
                # Seriously dog? We don't support anything can you just let it
                # go?
                self.msg(1, '>> IAC SB TN3270 TN3270E_FUNCTIONS '
                            'TN3270E_REQUEST IAC SE')
                self.send_data(IAC + SB + options['TN3270'] +
                               TN3270E_FUNCTIONS + TN3270E_REQUEST + IAC + SE)
            else:
                self.msg(1, "TN3270 Negotiation Complete!")
            # At this point we should be done negotiating options and recieve
            # tn3270 data with a tn3270e header
        return True
    ...
    # if reply < 0 and self.tn3270_header.request_flag ~= \
    #        self.TN3270E_RSF_NO_RESPONSE:
    ...
    def process_write(self, data):
        ...
        while i <= len(data) - 1:
            self.msg(2, "Current Position: " +
                     str(i) + " of " + str(len(data)))
            cp = data[i]
            ...
            if cp == SF:
                ...
                i = i + 1  # skip SF
                self.msg(2, "Writting Zero to buffer at address: %r",
                         self.buffer_address)
                ...
                self.buffer_address = \
                    self.DECODE_BADDR(struct.unpack(">B", data[i + 1])[0],
                ...
                # the current position is SBA, the next two bytes are the
                # lengths
                i = i + 3
                if len(data) > i:
                ...
                # Repeat address repeats whatever the next char is after the
                # two byte buffer address There's all kinds of weird GE stuff
                # we could do, but not now.
                # Maybe in future vers
                ...
                while (self.buffer_address != ra_baddr):
                    ...
                    self.buffer_address = self.INC_BUF_ADDR(
                        self.buffer_address)
            elif cp == EUA:
                ...
                while (self.buffer_address != eua_baddr):
                    # do nothing for now. this feature isn't
                    # supported/required at the moment we're technically
                    # supposed to delete the buffer but we might want to see
                    # whats on there!
                    self.buffer_address = self.INC_BUF_ADDR(
                        self.buffer_address)
            elif cp == GE:
            ...
            # But here is where Set Attribue is done. Things like Hidden and
            # Protected
                ...
                self.write_char(bytes([0o64]))
                self.buffer_address = self.INC_BUF_ADDR(self.buffer_address)
                i = i + 1
            else:  # whoa we made it.
                ascii_char = cp.decode('EBCDIC-CP-BE').encode('utf-8')
                self.msg(2, "Inserting " + ascii_char +
                         " (%r) at the following location:", data[i])
                self.msg(2, "  Row: %r", self.BA_TO_ROW(self.buffer_address))
                self.msg(2, "  Col: %r", self.BA_TO_COL(self.buffer_address))
                self.msg(2, "  Buffer Address: %r", self.buffer_address)
    ...
    def write_char(self, char):
        ...
        if self.buffer[self.buffer_address] == "\0":
            ...
            self.overwrite_buf[self.buffer_address] = \
                self.buffer[self.buffer_address]
            self.buffer[self.buffer_address] = char
    ...
    def send_tn3270(self, data):
        """Sends tn3270 data to the server. Adding 3270E options and doubling
        IACs"""
        packet = ''
        if self.state == TN3270E_DATA:
            ...
            # Since we're only in basic mode at the moment this is just a
            # skeleton. It will likely never be used.
    ...
    def w_structured_field(self, wsf_data):
        ...
        while bufflen > 0:
            ...
            fieldlen = (struct.unpack(">B", wsf_cmd[0])[
                        0] << 8) + struct.unpack(">B", wsf_cmd[1])[0]
            ...
            if (fieldlen < 3):
                ...
                return False
            if fieldlen > bufflen:
                self.msg(1, "error: field length", fieldlen,
                         " larger than buffer length %s", bufflen)
            ...
            if wsf_cmd[2] == SF_READ_PART:
                ...
                # rv_this = \
                #     self.sf_create_partition(wsf_cmd[3:fieldlen], fieldlen)
                # Do nothing for now other than print
    ...
    def read_partition(self, data):
        ...
        if len(data) < 2:
            ...
            return PDS_BAD_CMD
        self.msg(1, "[WSF] Partition ID " + ''.join(hex(ord(n))
                                                    for n in data[0]))
        if data[1] == SF_RP_QUERY:
            self.msg(1, "[WSF] Read Partition Query")
            if partition != bytes([0xff]):
            ...
            if self.state == TN3270E_DATA:
                query_options = ("\x00" * 5) + query_options
    ...
    def outbound_ds(self, data):
        ...
        if len(data) < 2:
            self.msg(1, "[WSF] error: field length %d too short", len(data))
            return PDS_BAD_CMD
        self.msg(1, "[WSF] Outbound DS value " +
                 ''.join(hex(ord(n)) for n in data[0]))
        if struct.unpack(">B", data[0])[0] != 0:
            self.msg(1, "OUTBOUND_DS: Position 0 expected 0 got %s", data[0])

        if data[1] == SNA_W:
            self.msg(1, "       - Write ")
            # skip the type value when we pass to process write
            ...
            self.msg(1, "unknown type " + ''.join(hex(ord(n))
                                                  for n in data[0]))
    ...
    def file_transfer(self, data):
        ...
        if data_type == TR_OPEN_REQ:
            ...
            if data_length == 35:
                ...
                # name = ""
                self.msg(
                    1, "[WSF] File Transfer: Open Recieved: Message: %s", name)
            elif data_length == 41:
                name = received_data[24:]
                recsz = self.ret_16(received_data[20:22])
                self.msg(1, "[WSF] File Transfer: Message Received: %s, "
                            "Size: %d", name, recsz)
            else:
            ...
            if self.message_flag:
                if received_data[0:7] == "TRANS03":
                    ...
                    self.msg(1, "[WSF] File Transfer: Message: %s",
                             received_data.strip())
                    self.ft_state = FT_NONE
                else:
                    self.msg(1, "[WSF] File Transfer: ERROR ERROR ERROR. "
                                "There was a problem.")
                    self.msg(1, "[WSF] File Transfer: Message: %s",
                             received_data)
                    self.ft_state = FT_NONE
                ...
                self.msg(1, "[WSF] File Transfer Insert: record number: %d | "
                            "bytes: %d", self.recnum, my_len)
                bytes_writen = 0
                for i in received_data:
                    if self.ascii_file and (i == "\r" or i == bytes([0x1a])):
                        continue
                    else:
                        bytes_writen += 1
                        self.file.write(i)
                self.msg(1, "[WSF] File Transfer Insert: Bytes Writen: %d",
                         bytes_writen)
            self.msg(1, "[WSF] File Transfer Insert: Data Ack: record number: "
                        "%d", self.recnum)
            self.output_buffer = []
            self.output_buffer.append(AID_SF)
            ...
            self.msg(1, "[WSF] File Transfer Current Buffer Size: %d",
                     self.ft_buffersize)
            ...
            while (not self.dft_eof) and (numbytes > 0):
                # Reading an ascii file and replacing NL with LF/CR
                if self.ascii_file:
                    self.msg(1, "[WSF] File Transfer ASCII: Reading one byte "
                    ...
                    self.msg(1, "[WSF] File Transfer Binary: Reading one byte "
                                "from %s", self.filename)
                    # Reads one byte from the file
                    # replace new lines with linefeed/carriage return
                    c = self.file.read(1)
                    ...
                    total_read = total_read + 1
            if(total_read > 0):
                self.msg(1, "[WSF] File Transfer: Record Number: %d | Sent %d "
                            "bytes", self.recnum, total_read)
                self.output_buffer.append(self.set_16(TR_GET_REPLY))
                self.output_buffer.append(self.set_16(TR_RECNUM_HDR))
                self.output_buffer.append(self.set_32(self.recnum))
                self.recnum = self.recnum + 1
                self.output_buffer.append(self.set_16(TR_NOT_COMPRESSED))
                self.output_buffer.append(bytes(TR_BEGIN_DATA))
                ...
                self.output_buffer.append(bytes(TR_ERROR_REPLY))
            ...
            for i in self.output_buffer:
                if len(i) == 0:
                    ...
                    o_len += len(i)
            # minus one because we shouldn't count AID_SF
            t_len = self.set_16(o_len - 1)
            ...
            self.msg(1, "[WSF] File Transfer: Insert")

        elif data_type == TR_SET_CUR_REQ:
            # We don't do anything here either
            self.msg(1, "[WSF] File Transfer: Set Cursor")
    ...
    def abort(self, code):
        ...
        self.output_buffer.insert(bytes(self.HIGH8(code)))
        self.output_buffer.insert(bytes(TR_ERROR_REPLY))
        self.output_buffer.insert(bytes(TR_ERROR_HDR))
        self.output_buffer.insert(bytes(TR_ERR_CMDFAIL))
        self.send_tn3270(self.output_buffer)
        self.output_buffer = []
        self.ft_state = FT_NONE

    # The following four functions allows for the sending and receiving of
    # files using IND$FILE
    #
    ...
    def send_ascii_file(self, dataset, filename):
        ...
        self.msg(1, "FILE TRANSFER: Writing %s to dataset %s at %s:%d in "
                    "ASCII format",
                 filename, dataset, self.host, self.port)
    ...
    def send_binary_file(self, dataset, filename):
        ...
        self.msg(1, "FILE TRANSFER: Writing %s to dataset %s at %s:%d",
                 filename, dataset, self.host, self.port)
        self.ft_state = FT_AWAIT_ACK
        self.ascii_file = False
        self.file = open(filename, "rb")
    ...
    def get_ascii_file(self, dataset, filename):
        ...
        self.msg(1, "FILE TRANSFER: Getting dataset %s from %s:%d writing to "
                    "%s as ASCII", dataset, self.host, self.port, filename)
        self.ft_state = FT_AWAIT_ACK
        self.ascii_file = True
    ...
    def get_binary_file(self, dataset, filename):
        """ Gets a dataset from the mainframe without
            any translation """
        self.msg(1, "FILE TRANSFER: Getting dataset %s from %s:%d writing "
                    "to %s", dataset, self.host, self.port, filename)
        self.ft_state = FT_AWAIT_ACK
        self.ascii_file = False
        self.file = open(filename, 'wb')  # file object
        self.filename = filename
    ...
    def send_cursor(self, data):
        ...
        output_addr += 1
        self.msg(1, "Output Address: %r", output_addr)
        self.msg(1, "Cursor Location (" + str(self.cursor_addr) +
                    "): Row: %r, Column: %r ",
        ...
        for lines in data:
            self.msg(1, 'Adding %r to the output buffer',
                     lines.decode('utf-8').encode('EBCDIC-CP-BE'))
            self.output_buffer.append(
                lines.decode('utf-8').encode('EBCDIC-CP-BE'))
        # --self.output_buffer[output_addr]  = \
        #       self:ENCODE_BADDR(self.cursor_addr + i)
        ...
        # --stdnse.debug(1, "sending the following: %s",
        #                stdnse.tohex(self.output_buffer))
    ...
    def writeable(self):
        ...
        for i in self.fa_buffer:
            if i != bytes([0x00]) and not (struct.unpack(">B", i)[0] & 0x20):
                # find next SFA:
                j_loc = 1
                for j in self.fa_buffer[b_loc + 1:]:
                    # print j
                    if j != bytes([0x00]) and (struct.unpack(">B", j)[0] & 0x20):
                        break
                    j_loc += 1
                self.msg(1, "Writeable Area: %d Row: %d Col: %d Length: %d",
                         b_loc, self.BA_TO_ROW(b_loc + 1),
